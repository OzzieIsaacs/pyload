...
class Package {
    ...
    data.links.forEach(link => {
      ...
      const html = `
        <span class='child_status'>
          <span style='margin-right: 2px;' class='${link.icon}'></span>
          </span>
  ...
  getLinkIcon(status) {
    switch (status) {
      ...
      case 4:
        return 'glyphicon glyphicon-arrow-right text-primary';
      case 11:
      case 13:
        return 'glyphicon glyphicon-cog text-info';
      default:
        return 'glyphicon glyphicon-cloud-download text-primary';
    }
  }
  ...
  movePackage(event) {
    ...
    $.post({
      ...
      dataType: 'json',
      contentType: 'application/json',      
      data: JSON.stringify({ pack_id: this.id, dest: ((this.ui.type + 1) % 2) }),
    })
      .done(() => {
        $(this.ele).remove();
        uiHandler.indicateFinish();
      ...
      .done((data) => {
        ...
        for (let i = 1; i <= length / 2; i++) {
          $.post({
            ...
            dataType: 'json',
            contentType: 'application/json',
            data: JSON.stringify({ file_id: data.links[length - i].fid, pos: i - 1 }),
            traditional: true
          }).fail(() => {
            uiHandler.indicateFail();
          });
